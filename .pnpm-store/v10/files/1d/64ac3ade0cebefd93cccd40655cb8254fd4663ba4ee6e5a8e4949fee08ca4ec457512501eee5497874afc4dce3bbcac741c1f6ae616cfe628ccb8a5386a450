...
function getSchemaVersionString(schema, api) {
  if (!_chunk4DQE5VZIcjs.isOAS31.call(void 0, api)) {
...
function toJSONSchema(data, opts = {}) {
  ...
  const schemaAdditionalProperties = _chunk4DQE5VZIcjs.isSchema.call(void 0, schema) ? schema.additionalProperties : null;
  const {
  ...
  if (_chunk4DQE5VZIcjs.isRef.call(void 0, schema)) {
    refLogger(schema.$ref, "ref");
    return transformer({
      $ref: schema.$ref
    });
  }
  if (_chunk4DQE5VZIcjs.isSchema.call(void 0, schema, isPolymorphicAllOfChild)) {
  ...
  if (_chunk4DQE5VZIcjs.isSchema.call(void 0, schema, isPolymorphicAllOfChild)) {
    ...
    if (hasSchemaType(schema, "array")) {
      if ("items" in schema) {
        if (!Array.isArray(schema.items) && Object.keys(schema.items).length === 1 && _chunk4DQE5VZIcjs.isRef.call(void 0, schema.items)) {
  ...
  if (_chunk4DQE5VZIcjs.isSchema.call(void 0, schema, isPolymorphicAllOfChild) && globalDefaults && Object.keys(globalDefaults).length > 0 && currentLocation) {
    try {
  ...
  if (_chunk4DQE5VZIcjs.isSchema.call(void 0, schema, isPolymorphicAllOfChild) && "enum" in schema && Array.isArray(schema.enum)) {
...
function getParametersAsJSONSchema(operation, api, opts) {
  ...
  const typeKeys = _chunkYHO3AOX6cjs.getExtension.call(void 0, _chunkYHO3AOX6cjs.PARAMETER_ORDERING, api, operation).map((k) => k.toLowerCase());
...
//# sourceMappingURL=chunk-TG4VCYD4.cjs.map